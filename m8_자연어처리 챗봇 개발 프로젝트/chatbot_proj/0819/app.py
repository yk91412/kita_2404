...
def index():
    ...
    if request.method == 'POST':
        ...
        # 채팅 기록을 관리하는 함수
        def update_chat_history(chat_history, message, is_user):
        ...
        # 사용자의 입력 추가
        chat_history = update_chat_history(chat_history, user_input, True)

        # 로딩 중 메시지 추가
        chat_history = update_chat_history(chat_history, "로딩 중...", False)
        ...
        # "로딩 중..." 메시지를 실제 응답으로 대체
        chat_history[-1] = {'message': bot_response, 'is_user': False}

        # 대화 기록 저장
        if user_id:
            save_chat(user_id, {'message': user_input, 'is_user': True})
            save_chat(user_id, {'message': bot_response, 'is_user': False})
        else:
...
def logout():
    ...
    session.pop('chat_history', None)  # 로그아웃 시 세션에서 채팅 기록을 삭제
    return redirect(url_for('index'))
