...
import requests
from database import db, ChatHistory,  User
from function import get_user, create_user, save_chat, \
get_summaries, get_chat_by_summary, summarize_text,\
...
def index():
    ...
    if request.method == 'POST':
        ...
        if user_id and session.get('show_date', False):
            ...
            logging.debug(f'Added date to chat history: {current_date}')
        ...
        logging.debug(f'Conversation input: {conversation_input}')
        try:
...
# 첫 대화 이후에만 요약 생성 및 저장
        if user_id :
            if len(chat_history) == 3:  # 첫 대화가 완료된 후에만 요약 저장
                latest_summary_id = db.session.query(db.func.max(ChatHistory.summary_id)).scalar()
            ...
            if user_id and summary_id:
                save_chat(user_id, user_input, 'user', summary_id, summary=question_summary)
                save_chat(user_id, bot_response_cleaned, 'bot', summary_id, summary=question_summary)


    # 클라이언트에 반환할 데이터
        response_data = {
            'user_input': user_input,
            'bot_response': bot_response_cleaned,
            'current_date': current_date if user_id and session.get('show_date', False) else None
        }

        return jsonify(response_data)
    
    # GET 요청 시에는 HTML 페이지를 렌더링
    return render_template('index.html', chat_history=chat_history, summaries=summaries)
...
def start_new_chat():
    if request.method == 'GET':
        ...
        if 'chat_history' not in session or not session['chat_history']:
            ...
            session.clear()
            print('초기화 완료 new chat')
    user_id = session.get('user_id')

    # 새로운 summary_id 생성
    new_summary_id = get_next_summary_id()
    logging.debug(f'New summary_id: {new_summary_id}')
...
#         if len(chat_history) >= 1 and session.get('show_date', '') == 'pending':
#             # 첫 질문이 추가된 후 날짜를 추가
#             current_date = datetime.now().strftime('%Y-%m-%d')
...
def delete_summary(summary_id):
    ...
    if user_id:
        ...
        summary = ChatHistory.query.filter_by(id=summary_id, user_id=user_id).first()
        if summary:
            # 관련된 채팅 기록도 삭제합니다.
