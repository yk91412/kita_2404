...
from database import User, ChatHistory, PasswordResetToken
...
def save_chat(user_id, message, role,summary_id,summary=None):
    if user_id:
        chat = ChatHistory(
            ...
            summary=summary  # 요약 정보를 포함합니다.
        )
        db.session.add(chat)
        try:
            db.session.commit()
            print(f"Chat saved successfully: {chat}")
...
def save_summary(user_id, date, summary):
    if user_id:
        summary_entry = Summary(user_id=user_id, timestamp=timestamp, summary=summary)
...
def get_summaries(user_id):
    ...
    """
    if user_id:
        return ChatHistory.query.filter_by(user_id=user_id, summary_id=summary_id).order_by(ChatHistory.timestamp.desc()).all()
    return []
