...
def index():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # bot_response = response_data['choices'][0]['message']['content'].strip().replace('\n','<br>')
...
def get_search_history():
    ...
    summaries = get_summaries(user_id)
    search_history = [{'id': s.id, 'date': s.date, 'summary': s.summary} for s in summaries]
...
def start_new_chat():
    user_id = session.get('user_id')
    if user_id:
        question_summary = "새 채팅이 시작되었습니다."  # 기본 메시지를 요약으로 저장
        # 새로운 summary_id 생성
        if len(chat_history) == 3:  # 첫 대화가 완료된 후에만 요약 저장
            if summary_id is None or (summary_id and summary_id < max([s.id for s in summaries], default=0)):
                current_date = datetime.now().strftime('%Y-%m-%d')
        ...
        # # 세션을 새로 초기화하여 새 채팅을 시작
        # session['summary_id'] = summary_id
        # session['chat_history'] = []  # 이전 대화 기록 초기화
        # session['first_question_asked'] = False  # 첫 질문이 아직 이루어지지 않았음을 표시

        return jsonify({'status': 'success', 'summary_id': summary_id}), 200
    return jsonify({'status': 'failure'}), 403
