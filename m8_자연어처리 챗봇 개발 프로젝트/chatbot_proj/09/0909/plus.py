...
def index():
    ...
    if request.method == 'POST':
        ...
        if final_response and final_response.choices:
            try:
                bot_response_openai = final_response.choices[0].message.content.strip()
            except (AttributeError, KeyError):
                bot_response_openai = "죄송합니다. 요청 처리 중 오류가 발생했습니다."
        else:
            bot_response_openai = "죄송합니다. 요청 처리 중 오류가 발생했습니다."

        # Langchain 대화 처리
        conversation_input = system_message + f"\n사용자: {user_input}\n"
        bot_response_langchain = conversation.run(input=conversation_input)
        bot_response_langchain_cleaned = bot_response_langchain.replace('\n', '<br>')

        # 두 응답을 결합하여 최종 응답 생성
        bot_response_cleaned = bot_response_openai  # 또는 bot_response_langchain_cleaned를 사용할 수 있음
        ...
        if user_id:
            ...
            if user_id and summary_id:
                ...
                save_chat(user_id, bot_response_cleaned, 'bot', summary_id)
