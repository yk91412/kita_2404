...
import requests
from database import db, get_user, create_user, save_chat, get_chat_history, ChatHistory, add_message_to_chat_history, save_summary, get_summaries, get_chat_by_summary, User
    ...
    if request.method == 'POST':
        ...
        if user_id:
            ...
            if len(chat_history) == 3:
                current_date = datetime.now().strftime('%Y-%m-%d')
                save_summary(user_id, current_date, user_input[:20])
