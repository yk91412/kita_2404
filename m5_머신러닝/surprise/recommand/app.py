...
ratings = pd.read_csv('ratings.csv', dtype={'userId': int, 'movieId': int, 'rating': float, 'timestamp': int})
movies = pd.read_csv('movies.csv', dtype={'movieId': int, 'title': str, 'genres': str})
...
def user_recommendations(userId):
    try:
        ...
        # If user ID not found or invalid input, show an error message
        movie_titles = ["해당하는 ID가 없거나 잘못된 입력입니다."]
...
def movie_recommendations(movie_title):
    ...
    if not movie_ids.empty:
        ...
        try:
            ...
            movie_titles = ["유사한 영화를 찾을 수 없습니다."]
    else:
        movie_titles = ["해당 영화 제목을 찾을 수 없습니다."]
...
def rating_recommendations(rating):
    try:
        ...
        return render_template('recommendations.html', recommendations=["잘못된 평점 입력입니다. 평점은 0.5에서 5 사이여야 합니다."])
